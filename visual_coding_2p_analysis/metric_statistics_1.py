...
cre_subset = ['Cux2-CreERT2','Rorb-IRES2-Cre','Rbp4-Cre_KL100','Ntsr1-Cre_GN220']
...
#        sns.heatmap(metric_array, cmap='seismic_r', linewidths=0.5, center=-1.3, 
#                    vmin=-4, cbar_kws={'label': 'log p value',
#                                       'ticks':[-3.3,-2.3,-1.3,-0.3,0.7]})
    if area_flag:
...
#    plt.title(title)
    fig.tight_layout()
    save_figure(fig, r'/Users/saskiad/Documents/CAM/paper figures/revision/stats/'+title)

def do_metric_stats(data_input, metric, ttest=False):
    metric_dict = metric_stats(data_input, metric, ttest=ttest)
    for a in areas:
        plot_p_values(metric_dict[a], metric+' '+a, area_flag=False)
    metric_cre = metric_stats_cre(data_input, metric, ttest=ttest)
    for c in cre_subset:
        plot_p_values(metric_cre[c], metric+' '+c.split('-')[0], area_flag=True)
    return metric_dict, metric_cre
