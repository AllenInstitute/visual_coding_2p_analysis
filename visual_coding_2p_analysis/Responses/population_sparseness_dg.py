...
save_path = os.path.join(save_path_head, 'DriftingGratings')
...
pop_sparse = np.empty((40,424))
pop_sparse[:] = np.NaN
pop_sparseness = pd.DataFrame(columns=('experiment_container_id','id','cre','area','depth_range','cre_depth','population_sparseness_dg','number_cells_dg'), index=range(424))
...
for index,row in exp.iterrows():
    ...
    a = row.id
    file_path = os.path.join(save_path, str(a)+'_dg_events_analysis.h5')
    f = h5py.File(file_path)
    response = f['response_events'].value
    f.close()
    response2 = response[:,1:,:,0].reshape(40,-1)

    num_cells = response2.shape[1]
    pop_sparseness['number_cells_dg'].loc[index] = num_cells
    
    ps = ((1-(1/float(num_cells))*((np.power(response2[:,:].sum(axis=1),2))/
                                      (np.power(response2[:,:],2).sum(axis=1))))/(1-(1/float(num_cells))))
    pop_sparse[:,index] = ps
pop_sparseness['population_sparseness_dg'] = np.nanmean(pop_sparse, axis=0)
...
for ai,a in enumerate(areas):
    for di,d in enumerate(cre_depth):
        ...
        results_cre[ai,di,0] = subset.population_sparseness_dg.median()
