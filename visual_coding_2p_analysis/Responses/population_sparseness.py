...
pop_sparse = np.empty((118,424))
pop_sparse[:] = np.NaN
pop_sparseness = pd.DataFrame(columns=('experiment_container_id','id','cre','area','depth_range','cre_depth','population_sparseness_ns','number_cells_ns'), index=range(424))
...
for index,row in exp.iterrows():
    ...
    a = row.id
    file_path = os.path.join(save_path, str(a)+'_ns_events_analysis.h5')
    f = h5py.File(file_path)
    response = f['response_events'].value
    f.close()

    num_cells = response.shape[1]
    pop_sparseness['number_cells_ns'].loc[index] = num_cells
    
    ps = ((1-(1/float(num_cells))*((np.power(response[1:,:,0].sum(axis=1),2))/
                                      (np.power(response[1:,:,0],2).sum(axis=1))))/(1-(1/float(num_cells))))
    pop_sparse[:,index] = ps
pop_sparseness['population_sparseness_ns'] = np.nanmean(pop_sparse, axis=0)
