...
save_path = os.path.join(save_path_head, 'NaturalMoviesA')

manifest_path = core.get_manifest_path()
from allensdk.core.brain_observatory_cache import BrainObservatoryCache
boc = BrainObservatoryCache(manifest_file = manifest_path)
exp = pd.DataFrame(boc.get_ophys_experiments(session_types=['three_session_A']))

pop_sparse = np.empty((3600,424))
pop_sparse[:] = np.NaN
pop_sparseness = pd.DataFrame(columns=('experiment_container_id','id','cre','area','depth_range','cre_depth','population_sparseness_nm3','number_cells_nm3'), index=range(424))
...
for index,row in exp.iterrows():
    ...
    a = row.id
    file_path = os.path.join(save_path, str(a)+'_nm_events_analysis.h5')
    f = h5py.File(file_path)
    response = f['response_events_3'].value
    f.close()

    num_cells = response.shape[1]
    pop_sparseness['number_cells_nm3'].loc[index] = num_cells
    
    ps = ((1-(1/float(num_cells))*((np.power(response[:,:,0].sum(axis=1),2))/
                                      (np.power(response[:,:,0],2).sum(axis=1))))/(1-(1/float(num_cells))))
    pop_sparse[:,index] = ps
pop_sparseness['population_sparseness_nm3'] = np.nanmean(pop_sparse, axis=0)
...
for ai,a in enumerate(areas):
    for di,d in enumerate(cre_depth):
        ...
        results_cre[ai,di,0] = subset.population_sparseness_ns.median()
